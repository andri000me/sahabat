...
<div class="container-fluid" style="margin-bottom:300px;">
    <form method="POST" class="needs-validation md-form text-left" style="color: #757575;" action="/koperasi/saveUploadPenolakan" enctype="multipart/form-data" novalidate>

        <input name="id" type="hidden" value="<?= $permohonan['idpermohonan'] ?>">
        <input name="slug" type="hidden" value="<?= $permohonan['slug'] ?>">
        <input name="img_penolakan_asal_lama" type="hidden" value="<?= $permohonan['img_penolakan_asal'] ?>">
        <input name="img_penolakan_tujuan_lama" type="hidden" value="<?= $permohonan['img_penolakan_tujuan'] ?>">

        <div class="my-4 px-3 py-3" style="width:1000px;">
            <h5 class="text-danger font-weight-bold card-title">Upload Surat Penolakan</h5>
                ...
                <div class="md-form">
                    <div class="file-field">
                        ...
                        if ($session['wilayah_id'] == $permohonan['asal']) {
                            if ($permohonan['img_penolakan_asal']) {
                                $link = 'href="/img/img_penolakan/' . $permohonan['img_penolakan_asal'] . '"';
                                $btn = "btn-success";
                            } else {
                                $link = "#";
                                $btn = "btn-danger";
                            ...
                            if ($permohonan['img_penolakan_tujuan']) {
                                $link = 'href="/img/img_penolakan/' . $permohonan['img_penolakan_tujuan'] . '"';
                                $btn = "btn-success";
                            } else {
                                $link = "#";
                                $btn = "btn-danger";
                        ...
                        <div class="file-path-wrapper">
                            <input name="img_penolakan" class="file-path validate" type="text" placeholder="Surat Penolakan">
                        </div>
                    </div>
            ...
            <div class="buttons mt-3">
                <button type="submit button" class="btn btn-md btn-danger text-light">Upload Surat Penolakan<i class="fa fa-check ml-1"></i> </button>
            </div>
        </div>
    </form>
</div>
